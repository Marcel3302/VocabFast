...
const expected=['app.js','grammar-data.js','index.html','styles.css','vocab-data.js'].sort();
...
for(const id of ['view-topics','view-words','view-practice','view-pdf','view-grammar','view-progress','view-account','selectAllLearn','selectAllPdf','selectVisiblePdfWords','toggleTopicSelection','newGrammarQuiz','newPractice','globalSort','rankPill'])if(!html.includes(`id="${id}"`))throw new Error(`missing #${id}`);
...
if(!wrangler.d1_databases?.some(x=>x.binding==='DB'))throw new Error('D1 DB binding missing');
...
for(const phrase of ['Alles abwählen','Sichtbare abwählen','10 neue Fragen','Cloud-Synchronisierung'])if(!app.includes(phrase)&&!html.includes(phrase))throw new Error(`Feature marker missing: ${phrase}`);
console.log(`Selftest OK: ${words.length} vocab entries (${c2} C2), ${grammar.length} grammar modules, cloud bindings, selection toggles, practice and static assets verified.`);
