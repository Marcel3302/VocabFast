import worker from '../src/worker-v4.js';
...
const env={PDFS:bucket,ASSETS:{fetch:()=>new Response('asset')},AI:{run:async()=>({response:JSON.stringify([{en:'refractory lining',de:'feuerfeste Ausmauerung',level:'B2',importance:5},{en:'firebrick',de:'Schamottestein',level:'B2',importance:5}])})}};
...
r=await call('/api/topics/generate',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:JSON.stringify({name:'Feuerfest Ausmauerung, Schamotte',count:80})}); x=await j(r); if(x.status!==200||!Array.isArray(x.data.words)||x.data.words.length<20||!x.data.words.some(w=>w.en==='refractory lining')) throw new Error('topic generation '+JSON.stringify(x));
...
r=await call('/api/state',{method:'PUT',headers:{Cookie:cookie,'Content-Type':'application/json'},body:JSON.stringify({learning:[{en:'rotor',de:'Rotor',level:'B1',learnStage:3}],customTopics:[],settings:{sort:'level-asc'},stats:{xp:10,studyDates:[]},mastery:{}})}); x=await j(r); if(!x.data.ok) throw new Error('state put');
