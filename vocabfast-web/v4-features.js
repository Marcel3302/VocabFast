...
function ensurePracticeControls(){
  ...
  const p=$('#view-practice .hero p');if(p)p.textContent='10 Fragen pro Runde – immer einzeln nacheinander. Lernstufe 3 = neu, 2 = lernen, 1 = sicher.';
}
...
window.addEventListener('load',async()=>{
  ...
  $('#newPractice')?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();startPractice()},true);
  for(const id of ['practiceMode','practiceLevel'])$('#'+id)?.addEventListener('change',e=>{e.stopImmediatePropagation();startPractice()},true);
