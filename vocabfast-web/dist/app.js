...
async function api(path, options={}){
  ...
  if(!res.ok){
    const err=new Error(data?.error || `Serverfehler ${res.status}`); err.status=res.status; err.code=data?.code||''; throw err;
  }
  return data ?? res;
}
...
function estimateCefr(){
  const s=state.stats||{}, levels=['A1','A2','B1','B2','C1','C2'];
  const counts=Object.fromEntries(levels.map(l=>[l,0])), mastered=Object.fromEntries(levels.map(l=>[l,0]));
  for(const w of state.learning||[]){
    if(counts[w.level]!==undefined) counts[w.level]++;
    if((state.mastery?.[uid(w)]||0)>=2 && mastered[w.level]!==undefined) mastered[w.level]++;
  }
  const totalAnswers=(s.vocabAnswered||0)+(s.grammarAnswered||0);
  const totalCorrect=(s.vocabCorrect||0)+(s.grammarCorrect||0);
  const accuracy=totalAnswers?totalCorrect/totalAnswers:0;
  const trained=Object.values(mastered).reduce((a,b)=>a+b,0);
  const exposure=state.learning.length||0;
  let score=0;
  score += Math.min(38, Math.log10(exposure+1)*12);
  score += Math.min(34, Math.log10(trained+1)*14);
  score += Math.min(20, totalAnswers/10);
  if(totalAnswers>=20) score += Math.max(-8,Math.min(8,(accuracy-.65)*40));
  const advanced=mastered.B2+mastered.C1+mastered.C2;
  if(advanced>=15)score+=5;if(advanced>=60)score+=7;if(mastered.C1+mastered.C2>=80)score+=8;if(mastered.C2>=80)score+=8;
  const gates=[0,24,39,55,72,88];
  let idx=0;for(let i=0;i<gates.length;i++)if(score>=gates[i])idx=i;
  if(totalAnswers<10&&trained<10)idx=Math.min(idx,1);
  if(totalAnswers<40&&trained<40)idx=Math.min(idx,2);
  const level=levels[idx];
  const confidence=totalAnswers>=100&&trained>=100?'gut':totalAnswers>=30&&trained>=30?'mittel':'niedrig';
  return {level,confidence,totalAnswers,trained,accuracy:Math.round(accuracy*100),score:Math.round(score),counts,mastered};
}
function renderCefrEstimate(){
  const e=estimateCefr(), badge=$('#cefrLevel'), title=$('#cefrTitle'), detail=$('#cefrDetail');
  if(!badge||!title||!detail)return;
  badge.textContent=e.level;
  title.textContent=`Geschätztes Niveau: ${e.level}`;
  detail.textContent=`Berechnet aus ${state.learning.length.toLocaleString('de-DE')} gespeicherten Wörtern, ${e.trained.toLocaleString('de-DE')} trainierten Wörtern und ${e.totalAnswers.toLocaleString('de-DE')} beantworteten Übungen. Datenbasis: ${e.confidence}${e.totalAnswers?` · Trefferquote ${e.accuracy}%`:''}.`;
}

function renderProgress(){
  renderRankUi();renderCefrEstimate();const s=state.stats,vAcc=s.vocabAnswered?Math.round((s.vocabCorrect/s.vocabAnswered)*100):0,gAcc=s.grammarAnswered?Math.round((s.grammarCorrect/s.grammarAnswered)*100):0;
...
$('#loginForm').onsubmit=async e=>{e.preventDefault();try{const data=await api('/auth/login',{method:'POST',body:{email:$('#loginEmail').value,password:$('#loginPassword').value}});await afterLogin(data.user);toast('Angemeldet');switchView('words');}catch(err){toast(err.message);}};
$('#registerForm').onsubmit=async e=>{e.preventDefault();try{const data=await api('/auth/register',{method:'POST',body:{name:$('#registerName').value,email:$('#registerEmail').value,password:$('#registerPassword').value}});await afterLogin(data.user);toast('Konto erstellt');switchView('words');}catch(err){toast(err.message);}};
...
async function init(){
  ...
  rebuildTopics();$('#globalSort').value=state.settings.sort||'importance-desc';renderAll();makeGrammarQuiz();switchView('words');
}
