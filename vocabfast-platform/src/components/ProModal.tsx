...
export default function ProModal({ onClose,user }: Props) {
  ...
  return <div className="pro-modal-backdrop" role="dialog" aria-modal="true" onMouseDown={onClose}>
    <section className="pro-modal" onMouseDown={event=>event.stopPropagation()}>
      ...
      {billing&&!billing.checkoutReady&&<p className="pro-test-note">Der Pro-Checkout ist für dieses System noch nicht freigeschaltet. Dein kostenloser Lernzugang bleibt uneingeschränkt nutzbar.</p>}
      {canCheckout&&<>
        <button className="pro-preview-action" disabled={opening} onClick={()=>void checkout()}>{opening?'Stripe wird geöffnet …':activePlan?'Abo sicher verwalten →':testMode?'Pro-Testkauf bei Stripe starten →':'Pro für 19,99 € / Monat kaufen →'}</button>
        {!activePlan&&<small className="pro-test-note">Nach dem erfolgreichen Stripe-Checkout wird der Pro-Zugang deinem angemeldeten VocabFast-Konto zugeordnet.</small>}
        {testMode&&<small className="pro-test-note">Stripe-Testmodus: Der Checkout funktioniert vollständig, es wird aber noch kein echtes Geld belastet.</small>}
      </>}
