...
export function PracticeView({ openLesson, buildReview, buildMode }: { openLesson: OpenLesson; buildReview: ReviewBuilder; buildMode: ModeBuilder }) {
  ...
  return <section className="platform-view">
    ...
    <div className="mode-grid">
      ...
      <button onClick={()=>openLesson(buildMode(['speaking'],'Sprechtraining','Kurze Sätze laut sprechen und direkt prüfen lassen.'))}><span className="mode-icon">🎙</span><div><strong>Sprechen</strong><p>Aktives Sprechen mit direkter Rückmeldung.</p></div><em>Mikrofon</em></button>
...
export function WordsView() {
  ...
  return <section className="platform-view">
    ...
    {undiscovered.length>0 && <><div className="view-section-head"><div><span className="eyebrow">ALS NÄCHSTES</span><h2>Weitere Konzepte im A1–C2 Kurs</h2></div></div><div className="locked-concepts">{undiscovered.slice(0,18).map(meta=><span key={meta.id}><b>{meta.label}</b><small>{meta.translation}</small></span>)}</div></>}
  </section>;
}

export function SpecialtyView({ openPro }: { openPro: () => void }) {
  return <section className="platform-view">
    <div className="view-hero specialty-hero"><div><span className="eyebrow">VOCABFAST PRO · FACHSPRACHE</span><h1>Alltagsenglisch reicht im Beruf oft nicht.</h1><p>Fachbereiche trainieren typische Situationen, relevanten Wortschatz, Hörverstehen, Sprechen und gezielte Kompetenztests.</p></div><div className="price-badge"><strong>19,99 €</strong><span>Pro / Monat</span></div></div>
    <div className="specialty-view-grid">{specialties.map(item=><article key={item.id}><div className="specialty-view-icon">{item.icon}</div><span className="pro-label">PRO</span><h2>{item.name}</h2><p>{item.description}</p><ul><li>situative Lektionen</li><li>Fachwortschatz, Hören & Sprechen</li><li>Kompetenztests</li></ul><button onClick={openPro}>Pro entdecken →</button></article>)}</div>
  </section>;
}
...
export function ProgressView({ progress }: { progress: PlatformProgress }) {
  ...
  return <section className="platform-view">
    <div className="view-hero compact"><div><span className="eyebrow">FORTSCHRITT</span><h1>Du siehst nicht nur, was erledigt ist – sondern was sitzt.</h1><p>Dein Fortschritt über A1 bis C2 setzt sich aus Lektionen, Genauigkeit, Streak und Konzeptstärke zusammen.</p></div></div>
    ...
    <div className="view-section-head"><div><span className="eyebrow">KURSSTATUS</span><h2>A1 bis C2</h2></div></div>
    <div className="unit-progress-grid">{englishCourseLevels.map(level=>{const lessons=level.units.flatMap(unit=>unit.lessons);const complete=lessons.filter(lesson=>progress.completedLessonIds.includes(lesson.id)).length;const percent=lessons.length?Math.round(complete/lessons.length*100):0;return <article key={level.id}><div><span>{level.id}</span><strong>{level.title}</strong><small>{complete}/{lessons.length} Lektionen</small></div><div className="unit-progress-ring"><strong>{percent}%</strong></div></article>})}</div>
