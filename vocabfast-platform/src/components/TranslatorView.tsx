...
export default function TranslatorView({sourceLanguage,targetLanguage,onSwap}:Props){
  ...
  const providerLabel=result?.provider==='standard'?'Standard-Übersetzung':result?.provider==='ai-fallback'?'KI-Fallback':result?.provider==='history'?'Aus Verlauf':'Übersetzung';
