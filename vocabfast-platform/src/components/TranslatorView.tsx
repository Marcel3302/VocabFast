...
export default function TranslatorView({sourceLanguage,targetLanguage,onSwap}:Props){
  ...
  const [text,setText]=useState('');const [result,setResult]=useState<TranslationResult|null>(null);const [busy,setBusy]=useState(false);const [error,setError]=useState('');const [saved,setSaved]=useState('');
  ...
  function resetResult(){setResult(null);setError('');setSaved('');}
  async function run(){if(!text.trim()||source===target||busy)return;setBusy(true);setError('');setSaved('');try{setResult(await translate(text,source,target));}catch(reason){setError(reason instanceof Error?reason.message:'Übersetzung fehlgeschlagen.');}finally{setBusy(false);}}
  ...
  function saveResult(){
    ...
    const current=readWords(),next=mergeWords(current,[makeWord(sourceText,result.translation,'Übersetzer')]);
    if(next.length===current.length){setSaved('Bereits in deinem Wortschatz.');return;}
    try{saveWords(next);setSaved('✓ Als Karteikarte gespeichert.');setError('');}catch{setSaved('');setError('Die Karteikarte konnte nicht gespeichert werden.');}
  }
  return <section className="platform-view translator-view">
    ...
    <div className="translator-card">
      ...
      <div className="translator-grid"><div className="translator-input"><div className="translator-pane-head"><strong>{sourceMeta?.name}</strong><small>{text.length}/3000</small></div><textarea maxLength={3000} value={text} onChange={event=>{setText(event.target.value);setSaved('');}} onKeyDown={event=>{if((event.ctrlKey||event.metaKey)&&event.key==='Enter'){event.preventDefault();void run();}}} placeholder="Text eingeben …"/><div className="translator-actions"><small>Strg/⌘ + Enter</small><button onClick={()=>{setText('');resetResult();}} disabled={!text}>Leeren</button><button className="view-primary" onClick={()=>void run()} disabled={busy||!text.trim()||source===target}>{busy?'Übersetze …':'Übersetzen →'}</button></div></div><div className="translator-output"><div className="translator-pane-head"><strong>{targetMeta?.name}</strong><small>{result?'Übersetzung':'Bereit'}</small></div>{error?<div className="translator-error">{error}</div>:result?<div className="translator-result"><h2>{result.translation}</h2>{result.alternatives.length>0&&<div className="translator-alternatives"><span>Natürliche Varianten</span>{result.alternatives.map(value=><button key={value} onClick={()=>navigator.clipboard?.writeText(value)}>{value}</button>)}</div>}{result.note&&<div className="translator-note"><span>LERNHINWEIS</span><p>{result.note}</p></div>}<div className="translator-result-actions"><button className="translator-copy" onClick={()=>navigator.clipboard?.writeText(result.translation)}>Übersetzung kopieren</button><button className="translator-save" onClick={saveResult} disabled={text.trim().length>200||result.translation.length>500}>+ Als Karteikarte speichern</button></div>{saved&&<div className="translator-saved" role="status">{saved}</div>}</div>:<div className="translator-empty"><strong>{sourceMeta?.symbol} → {targetMeta?.symbol}</strong><span>Deine Übersetzung erscheint hier.</span></div>}</div></div>
    </div>
  </section>;
}
