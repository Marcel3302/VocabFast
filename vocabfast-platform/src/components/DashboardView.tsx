...
export default function DashboardView({ progress, preferences, activeLevel, placement, lastResult, openLesson, buildReview, openPro, openCourse, openPlacement, onSelectLevel }: Props) {
  ...
  return <div className="page-grid">
    <section className="primary-column">
      <div className="hero-panel">
        <div className="hero-copy">
          ...
          <p>{level.goal} Dein Lernstand verbindet Lektionen, Grammatik, Hören, Sprechen und adaptive Wiederholung zu einem persönlichen Trainingsplan.</p>
      ...
      <div className="dashboard-course-callout"><div><strong>Dein Lernpfad reicht von A1 bis C2.</strong><p>Wähle ein Level frei oder nutze die 36-Fragen-Einstufung für eine persönliche Start- und Fokus-Empfehlung.</p></div><div className="hero-actions"><button onClick={openPlacement}>Niveau bestimmen</button><button onClick={openCourse}>Gesamtkurs →</button></div></div>

      {placement&&<section className="personal-plan-card">
        <div className="personal-plan-head"><div><span className="eyebrow">DEIN PERSÖNLICHER STARTPLAN</span><h2>{placement.recommendedLevel} empfohlen · {placement.score}/{placement.total} richtig</h2><p>Deine Einstufung zeigt dir, wo du starten solltest und welche Bereiche aktuell den größten Nutzen bringen.</p></div><div className="personal-plan-level"><span>START</span><strong>{placement.recommendedLevel}</strong><small>{Math.round((placement.score/Math.max(1,placement.total))*100)}% gesamt</small></div></div>
      ...
      <div className="section-heading" id="course-units"><div><span className="eyebrow">DEIN WEG</span><h2>Englisch · {activeLevel} {level.title}</h2></div><span className="pro-label">{lessons.length} Lektionen</span></div>
    ...
    <aside className="right-column">
      ...
      <section className="coach-card"><div className="coach-head"><div className="coach-mark">AI</div><div><span className="eyebrow">VOCABFAST COACH</span><h3>{progress.sessions?'Dein Training wird persönlicher.':'Starte mit einer ersten Session.'}</h3></div></div><p>Coach und Wiederholung orientieren sich an deinem CEFR-Level und deinen bisherigen Lernschwerpunkten.</p><div className="coach-insight"><span>Aktueller Fokus</span><strong>Englisch {activeLevel}</strong><small>{level.descriptor}</small></div><button onClick={()=>openLesson(progress.sessions?buildReview():lessons[0])}>Training starten</button></section>
      ...
      <section className="pro-card"><span className="pro-pill">VOCABFAST PRO</span><h3>Dein persönlicher Sprachcoach.</h3><p>KI-Training, Aussprache, Fachbereiche, Dokumentlernen und tiefere Analyse.</p><div className="price-row"><strong>19,99 €</strong><span>/ Monat</span></div><button onClick={openPro}>Pro entdecken</button></section>
    </aside>
  </div>;
}
