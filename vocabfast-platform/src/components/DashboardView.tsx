...
export default function DashboardView({ progress, preferences, activeLevel, lastResult, openLesson, buildReview, openPro, openCourse, openPlacement, onSelectLevel }: Props) {
  ...
  const courseProgress = Math.round((curriculumCompleted / lessons.length) * 100);
    ...
    <section className="primary-column">
      <div className="hero-panel">
        <div className="hero-copy">
          ...
          <p>{level.goal} Der aktuelle Entwicklungsstand verbindet kurze Lektionen mit Hören, Sprechen und adaptiver Wiederholung.</p>
          <div className="hero-actions"><button className="primary-action" onClick={()=>openLesson(nextLesson)}>Weiterlernen <span>→</span></button><button className="secondary-action" onClick={openCourse}>A1–C1 Kursplan</button></div>
        </div>
        <div className="hero-orbit" aria-hidden="true"><div className="orbit-ring ring-one"/><div className="orbit-ring ring-two"/><div className="core-word"><span>{activeLevel}</span><strong>{courseProgress}%</strong><small>{curriculumCompleted}/{lessons.length} Lektionen</small></div><div className="orbit-dot dot-one">A</div><div className="orbit-dot dot-two">✓</div><div className="orbit-dot dot-three">🎙</div></div>
      </div>
      ...
      <div className="dashboard-course-callout"><div><strong>A1 bis C1 ist jetzt als spielbarer Kursrücken vorhanden.</strong><p>Wähle ein Level frei zum Testen oder nutze die schnelle Einstufung.</p></div><div className="hero-actions"><button onClick={openPlacement}>Einstufung</button><button onClick={openCourse}>Gesamtkurs →</button></div></div>
      ...
      <div className="adaptive-strip"><div><strong>{activeLevel} · Adaptive Wiederholung</strong><span>VocabFast nutzt nur Konzepte des aktuell gewählten Levels und priorisiert Fälligkeiten und Schwächen.</span></div><button onClick={()=>openLesson(buildReview())}>Intelligente Übung starten</button></div>
