...
type ReaderState={lastPage:number;bookmarks:number[];notes:Record<string,string>};
...
function readState(id:string):ReaderState{
  try{
    const raw=JSON.parse(localStorage.getItem(stateKey(id))||'{}') as Partial<ReaderState>;
    const notes=raw.notes&&typeof raw.notes==='object'?Object.fromEntries(Object.entries(raw.notes).filter(([,value])=>typeof value==='string').map(([key,value])=>[key,String(value).slice(0,4000)])):{};
    return{lastPage:Math.max(1,Number(raw.lastPage)||1),bookmarks:Array.isArray(raw.bookmarks)?raw.bookmarks.filter(value=>Number.isInteger(value)&&value>0):[],notes};
  }catch{return{lastPage:1,bookmarks:[],notes:{}};}
}
...
export default function PDFReader({sourceLanguage,targetLanguage,onClose}:Props){
  ...
  },[page,scale,totalPages,documentVersion]);
  useEffect(()=>{if(readerIdRef.current&&totalPages)writeState(readerIdRef.current,{lastPage:page,bookmarks,notes});},[page,bookmarks,notes,totalPages]);
  useEffect(()=>{
    const onKey=(event:KeyboardEvent)=>{
      const targetElement=event.target as HTMLElement|null;
      const editing=targetElement?.tagName==='INPUT'||targetElement?.tagName==='TEXTAREA'||targetElement?.tagName==='SELECT'||targetElement?.isContentEditable;
      if(event.key==='Escape'&&!editing){event.preventDefault();close();return;}
      if(!totalPages||editing)return;
      if(event.key==='ArrowLeft'){event.preventDefault();jump(page-1);}
      if(event.key==='ArrowRight'){event.preventDefault();jump(page+1);}
      if(event.key.toLowerCase()==='b'){event.preventDefault();toggleBookmark();}
    };
  ...
  async function loadPdf(file:File){
    ...
    try{
      ...
      const id=signature(file),saved=readState(id);readerIdRef.current=id;setFileName(file.name);setTotalPages(pdf.numPages);setBookmarks(saved.bookmarks.filter(value=>value<=pdf.numPages));setNotes(saved.notes);setPage(Math.min(saved.lastPage,pdf.numPages));setDocumentVersion(value=>value+1);
  ...
  function updateNote(value:string){setNotes(current=>{const next={...current};const trimmed=value.slice(0,4000);if(trimmed)next[String(page)]=trimmed;else delete next[String(page)];return next;});}
