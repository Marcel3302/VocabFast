...
async function memoryTranslate(text,source,target){
  ...
  for(const chunk of split(text)){
    ...
    if(split(text).length===1&&Array.isArray(data?.matches))alternatives=data.matches.map(item=>clean(entities(item?.translation),800)).filter(Boolean);
  }
  ...
  return{translation,alternatives,note:'Automatische Übersetzung. Bei Fachbegriffen kann der richtige Ausdruck vom Satzkontext abhängen.',source,target,provider:'standard'};
}
...
async function aiTranslate(text,source,target,env){
  ...
  const result=await env.AI.run(env.AI_CHAT_MODEL||'@cf/meta/llama-3.1-8b-instruct',{messages:[{role:'system',content:system},{role:'user',content:text}],max_tokens:650,temperature:0});
...
export async function robustTranslateApi(request,env,baseWorker,{requirePro=false}={}){
  ...
  try{return json(await memoryTranslate(text,source,target));}catch(first){console.warn('standard translation failed',String(first));}
  try{return json(await aiTranslate(text,source,target,env));}catch(second){console.error('translation fallback failed',String(second));}
