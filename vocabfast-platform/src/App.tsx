...
export default function App(){
  ...
  const navItems=useMemo(()=>englishMode?[['home','Lernpfad'],['course',`${courseRange} Kurs`],['grammar','Grammatik'],['practice','Üben'],['coach','Coach'],['translate','Übersetzen'],['words','Wortschatz'],['specialty','Fachsprache'],['progress','Fortschritt']] as const:[['home','Lernpfad'],['course',`${targetMeta.name} ${courseRange}`],['practice','Üben'],['translate','Übersetzen'],['progress','Fortschritt']] as const,[englishMode,courseRange,targetMeta.name]);
  ...
  function openNavigation(id:NavId){if(englishMode&&accountUser?.plan!=='pro'&&(id==='coach'||id==='specialty')){setMobileMoreOpen(false);setProOpen(true);return;}navigateTo(id);}
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <div className="brand-row"><div className="brand-mark">V</div><div><strong>VocabFast</strong><span>Sprachtraining</span></div></div>
      ...
      <nav className="main-nav" aria-label="Hauptnavigation">{navItems.map(([id,label])=>{const proFeature=englishMode&&accountUser?.plan!=='pro'&&(id==='coach'||id==='specialty');return <button key={id} className={activeNav===id?'active':''} onClick={()=>openNavigation(id as NavId)}><span className="nav-icon">{id==='course'?courseState.activeLevel:icons[id]}</span><span>{label}</span>{proFeature&&<small className="nav-pro-badge">PRO</small>}</button>})}</nav>
      <div className="sidebar-spacer"/>
      {englishMode&&(accountUser?.plan==='pro'?<div className="pro-mini-card pro-active"><button type="button" className="pro-pill pro-link" onClick={()=>setProOpen(true)}>PRO AKTIV</button><strong>Dein Pro-Zugang ist aktiv.</strong><p>Coach, Fachsprache und intensives Training stehen dir zur Verfügung.</p><button onClick={()=>setProOpen(true)}>Abo verwalten</button></div>:<div className="pro-mini-card"><button type="button" className="pro-pill pro-link" onClick={()=>setProOpen(true)}>PRO</button><strong>Mehr aus jeder Minute.</strong><p>KI-Coach, Fachsprache, Analyse und intensives Training.</p><button onClick={()=>setProOpen(true)}>Pro entdecken</button></div>)}
    ...
    <Suspense fallback={(lessonOpen||onboardingOpen||placementOpen||proOpen)?<ModalLoading/>:null}>
      ...
      {proOpen&&englishMode&&<ProModal user={accountUser} onClose={()=>setProOpen(false)}/>} 
    </Suspense>
  </div>;
}
