...
export default function App(){
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav className="main-nav" aria-label="Hauptnavigation">{navItems.map(([id,label])=>{const proFeature=englishMode&&!isPro&&(id==='coach'||id==='specialty');return <button key={id} className={activeNav===id?'active':''} onClick={()=>openNavigation(id as NavId)}><span className="nav-icon">{id==='course'?courseState.activeLevel:icons[id]}</span><span>{label}</span>{proFeature&&<small className="nav-pro-badge">PRO</small>}</button>})}</nav>
      <div className="sidebar-spacer"/>
      {englishMode&&!isPro&&<div className="pro-mini-card"><button type="button" className="pro-pill pro-link" onClick={()=>setProOpen(true)}>PRO</button><strong>Mehr aus jeder Minute.</strong><p>KI-Coach, Fachsprache, Analyse und intensives Training.</p><button onClick={()=>setProOpen(true)}>Pro entdecken</button></div>}
